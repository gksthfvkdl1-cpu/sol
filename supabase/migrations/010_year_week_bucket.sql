-- 주차 버킷을 "1주차=1/1~1/7" 규칙으로 통일

CREATE OR REPLACE FUNCTION public.year_week_start(p_ts timestamptz)
RETURNS DATE
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT (
    date_trunc('year', p_ts)::date
    + (floor((extract(doy FROM p_ts) - 1) / 7)::int * 7)
  )::date;
$$;
...
AS $$
  WITH target AS (
    SELECT public.year_week_start(p_week_start::timestamptz) AS ws
  ),
  per_match AS (
    SELECT
      m.id,
...
GRANT EXECUTE ON FUNCTION public.year_week_start(timestamptz) TO anon;
